...
- cycle 200 先覆盖安装提交 `971c2753` 的最终 arm64 Debug APK，再执行相同直播矩阵并真实退出 0；14/14 门禁继续通过，远端可见弹幕为 10 条，系统 PiP 返回后弹幕页仍存活，日志无 FATAL/ANR。APK 为 299,150,717 B，SHA-256 `B0EEAF3434E961EFD10419BEF59AC46164D44CDD61DC5746CCE63C3AFFF259DF`；构建记录 `local-artifacts/build-records/20260904T193151421Z-build-androidarm64-debug.json`，实机证据 `local-artifacts/diagnostics/android-runtime-smoke-20260905T033502392/summary.json`。包装器取得 C 轮后自动唤醒并解除无密码锁屏，结束时恢复 10 分钟锁屏并交棒给 A 轮。
...
- 通用呈现门禁 `tool/android_presentation_smoke.ps1` 在 cycle 205 完成当前抖音原生竖屏房间的真实闭环。普通直播页同时出现明确的下滑竖屏沉浸与横屏全屏入口；下滑后维持 `1200×2608` 并显示上滑恢复提示，恢复弹幕栏后进入 `2608×1200` 横屏全屏，截图确认竖向主体完整居中且两侧为同源环境背景；系统返回恢复普通直播页，系统 PiP 保持竖向比例，重新进入应用后仍为同一房间并继续显示弹幕。9 项运行断言、方向和 FATAL 检查全部通过；证据：`local-artifacts/diagnostics/android-portrait-presentation-20260905T040128729/summary.json`。
- 同一门禁的 `Standard` 模式在 cycle 206 选中当前 Bilibili 普通横向直播，确认普通页没有进入竖屏专用路径；显式进入全屏后设备由 `1200×2608` 切到 `2608×1200`，实图显示标准画面完整铺满。Android 系统返回先恢复普通直播页，系统 PiP 保持横向比例，恢复应用后仍为同一房间且弹幕 UI 存活；7 项模式断言与 FATAL 检查全部通过。证据：`local-artifacts/diagnostics/android-standard-presentation-20260905T040738909/summary.json`。
...
- 通用门禁重构后的 cycle 207 又稳定暴露了一个独立产品问题：竖屏沉浸的 104 dp 底部控制栏在 Stack 中绘制于全表面亮度/音量手势层之上，而恢复判定只存在于下层最后 96 dp；控制栏可见时，上滑会由上层命中，页面继续停留在带“退出全屏”的沉浸模式。首个错误状态发生在命中测试层级，不在画面识别、播放器、方向或系统导航。
- 修复只在 `VideoMode.portraitFullscreen` 为底部控制栏包裹纵向恢复识别器；子按钮的点击继续工作，达到 64 dp 距离或有界快速上滑才退出沉浸，普通/横屏控制栏不注册该手势。Widget 回归覆盖“控制按钮可点 + 同一区域上滑恢复”和禁用模式无副作用，共 6/6，通过后单次 Analyze 为 0 issue；质量记录 `local-artifacts/build-records/20260904T201800673Z-quality-focused.json`。
- 提交 `6e1deea1` 的 arm64 Debug APK 构建用时 77.119 秒，产物 299,154,225 B，SHA-256 `69B06C736A3A040026F25F3D27BE26AA21F58E60A5CC978E13FC50F02BF2D926`，16 个 arm64 原生库最小 ELF LOAD 对齐均不低于 `0x4000`；构建记录 `local-artifacts/build-records/20260904T202013220Z-build-androidarm64-debug.json`。cycle 208 覆盖安装后完整竖屏呈现门禁恢复为 9/9，通过普通页、竖屏沉浸、可见控制栏上滑恢复、横屏环境背景、系统返回、竖向 PiP、回房与 FATAL 检查；证据 `local-artifacts/diagnostics/android-portrait-presentation-20260905T042024096/summary.json`。
- cycle 209 用同一 APK 复验普通横向流的相邻模式，Bilibili 当前房间的普通页没有竖屏手势，横屏全屏完整铺满 `2608×1200`，系统返回、横向 PiP、回房与 FATAL 检查全部通过；这同时证明新增手势包装器在非竖屏沉浸模式不注册纵向手势。证据 `local-artifacts/diagnostics/android-standard-presentation-20260905T042627364/summary.json`。
- #846 的设备等价样本由 `tool/android_favorite_bucket_smoke.ps1` 在 cycle 210 复核。脚本等待冷启动刷新完成后依次选择“未开播”和“虎牙”，语义树确认两个标签均处于选中态；页面实际显示两个虎牙未开播收藏的封面、标题和主播，不存在“关注数量大于零但列表为空”，日志无 FATAL。该结果与 8 个当前开播房间的串行/4 路并发接口探针共同说明当前分支没有复现报告症状，同时继续保留报告者未提供原房间集合这一证据边界。证据 `local-artifacts/diagnostics/android-favorite-offline-20260905T043215093/summary.json`。
...
- cycle 212 在最终 Debug APK 的当前虎牙房间真实执行 `蓝光30M → 流畅` 与 `线路1 → 线路2`。两个选择分别在 UIAutomator 观察的 2,940 ms、2,942 ms 内提交，三秒后的稳定快照仍保留新标签，7 条真实弹幕继续更新；随后在同一房间使用录制器独立选择的源完成短录，得到 16,380,319 B、32.004667 秒、H.264 + AAC 的 MP4，SHA-256 `63F8759FE678705006F645B9D8C9C9BE882B057059E0A15F6CF9FF88A12BE508`。32/32 门禁、监控移除、进程退出、活动 Wake Lock 清理及 FATAL 过滤全部通过；证据 `local-artifacts/diagnostics/android-recording-smoke-20260905T044047480/summary.json`。
...
- cycle 216 的抖音样本真实执行 `高清 → 标清` 与 `线路1 → 线路2`，分别在 2,994 ms、2,978 ms 内提交，稳定快照没有回退；本轮房间安静但弹幕连接成立。切换后的短录为 10,321,701 B、28.933333 秒、H.264 + AAC，共 31 项门禁（含 4 个真实切换门禁）通过；证据 `local-artifacts/diagnostics/android-recording-smoke-20260905T045522555/summary.json`。
...
## 2026-09-05 单任务补充：网易 CC 与 YY

用户暂停共享轮转后，通过包装器 `-NoRotation` 执行，保持唤醒与 finally 清理；测试包仍为此前已安装的 3.1.8 Debug，不是本轮控制器竞态修复的新包。

- 网易 CC：`原画 → 高清`、`线路1 → 线路2` 标签提交并稳定；31 项断言通过。MP4 5,134,204 B / 36.999367 秒，音视频轨道可读，录制期间文件增长，停止后进程与 Wake Lock 清理正常。证据：`local-artifacts/diagnostics/android-recording-smoke-20260905T051327904/summary.json`。
