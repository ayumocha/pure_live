# v3.0.8 独立同步候选（未发布，2026-09-25）

- 在 `codex/sync-upstream-20260925` 同步上游 `9b376ec9ef25` 的完整历史和经审查实现，候选版本为 `3.0.8+4096`。
- 斗鱼采用上游直播源刷新、恢复与 source commit 机制；旧本地恢复补丁仅保存在备份分支。
- 保留维护分支更新源，迁移旧 `xhs` 收藏、历史、目录、偏好、音量和竖屏配置，兼容旧小红书主播主页与动态链接入口。
- 启用实际包含原生纹理/Surface 修补的本地播放器依赖，固定依赖版本并升级 Flutter 至 `3.47.5`。
- 暂缓上游未认证的局域网远程同步；本地备份、WebDAV 和电视扫码继续保留。
- 以下上游版本条目是历史资料，不表示此 fork 已发布对应版本。本候选的实际验证与限制见 `docs/UPSTREAM_AUDIT_9b376ec9ef25.md`。
