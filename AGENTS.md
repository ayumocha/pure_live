...
- Current master includes the accepted upstream integration through `e6f20091`, with frozen upstream `9b376ec9ef25b532296871073a39075f6f9db397`, the blocked-mirror fix, and the manual loudness compensation feature from `0b4f7032`. Windows `3.0.9+4097` release preparation is in progress; macOS remains on `3.0.8+4096`, and Android/Linux/iOS remain on `3.0.7+4095`. Read [the current release status](docs/BUILD_AND_RELEASE.md), [loudness validation](docs/LOUDNESS_COMPENSATION.md), and [the historical sync result](docs/UPSTREAM_SYNC_RESULT_2026_09_25.md); historical handoff test and device claims do not validate this checkout.
...
- Release preparation baseline: `3.0.9+4097`. The opening sections of `RELEASE_NOTES.md` describe this fork's releases; the explicitly marked upstream history does not establish that this fork published those versions. Read `docs/BUILD_AND_RELEASE.md` for publication status, and increment future versions from `pubspec.yaml`.
