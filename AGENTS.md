...
- Current master includes the accepted upstream integration through `e6f20091`, with frozen upstream `9b376ec9ef25b532296871073a39075f6f9db397`, the blocked-mirror fix, and the manual loudness compensation feature from `0b4f7032`. Windows `3.0.10+4098` is published as [v3.0.10](https://github.com/ayumocha/pure_live/releases/tag/v3.0.10) from `74bbb889`, removing third-party proxy sources and unsolicited startup writes; macOS remains on `3.0.8+4096`, and Android/Linux/iOS remain on `3.0.7+4095`. Read [the current release status](docs/BUILD_AND_RELEASE.md), [startup alert fixes and validation](docs/KASPERSKY_STARTUP_3_0_10.md), [loudness validation](docs/LOUDNESS_COMPENSATION.md), and [the historical sync result](docs/UPSTREAM_SYNC_RESULT_2026_09_25.md); historical handoff test and device claims do not validate this checkout.
...
- Released Windows line: `3.0.10+4098`. The opening sections of `RELEASE_NOTES.md` describe this fork's releases; the explicitly marked upstream history does not establish that this fork published those versions. Read `docs/BUILD_AND_RELEASE.md` for publication status, and increment future versions from `pubspec.yaml`.
