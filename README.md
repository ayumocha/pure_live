...
v3.0.9 新增 **手动响度补偿**：在“播放设置 → 音频设置”中选择关闭、轻柔（+3 dB）、标准（+6 dB）或强力（+9 dB）。默认关闭，建议从轻柔开始；MPV 主播放器及多画面共用此设置，普通音量和静音仍有效。这是手动增益，不会自动统一各直播间响度，也没有自动限幅；较响音源使用高档位可能失真。[设计与验证记录](docs/LOUDNESS_COMPENSATION.md)
