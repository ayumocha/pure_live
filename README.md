...
> **小红书直播（链接观看）**：小红书未向客户端提供免签名的房间列表与实时弹幕接口，因此本应用以"链接观看"方式接入——粘贴直播间地址（`xiaohongshu.com/livestream/{房间号}`）或主播主页链接（`xiaohongshu.com/user/profile/{主播ID}`）后，可解析直播状态、标题、封面、主播、人数与多档清晰度并直接播放；支持收藏、历史与录制。"网页搜索"页右上角链接按钮可粘贴任意已支持平台的直播间链接。
...
开发分支新增 **手动响度补偿**（尚未包含在 v3.0.8 正式版）：在“播放设置 → 音频设置”中选择关闭、轻柔（+3 dB）、标准（+6 dB）或强力（+9 dB）。默认关闭，建议从轻柔开始；MPV 主播放器及多画面共用此设置，普通音量和静音仍有效。这是手动增益，不会自动统一各直播间响度，也没有自动限幅；较响音源使用高档位可能失真。[设计与验证记录](docs/LOUDNESS_COMPENSATION.md)
...
- 小红书直播间链接识别（`livestream/{房间号}`、`livestream/{动态段}/{房间号}`、`user/profile/{主播ID}`，支持"网页搜索"页右上角链接按钮粘贴打开）

**工具箱 → 链接解析**同样支持小红书（含 xhslink.com 短链自动跳转与分享文本内链接提取）；未开播/已结束的房间会提示主播信息并支持一键加入关注，开播后即可从关注列表进入观看。
